    ...
    def generate_svg(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            chart_svg_generator = KerykeionChartSVG(
                self.natal_subject,
                second_obj=self.transit_subject if chart_type in ["transit", "combined"] else None,
                chart_type=self.CHART_TYPES[chart_type]
            )

            # Configurar diretório de saída
            if hasattr(chart_svg_generator, 'set_output_directory'):
                chart_svg_generator.set_output_directory(temp_dir)

            # Configurar tema se suportado
            if hasattr(chart_svg_generator, 'set_up_theme') and theme == "dark":
                chart_svg_generator.set_up_theme("dark")

            # Configurar idioma se suportado
            if hasattr(chart_svg_generator, 'set_language') and language:
                chart_svg_generator.set_language(language)

            # Gerar SVG
            try:
                if hasattr(chart_svg_generator, 'makeSVG'):
